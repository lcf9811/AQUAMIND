...
class KnowledgeBase:
    ...
    def _add_turntable_knowledge(self):
        ...
        # 转盘频率控制
        for i in range(1, 4):
            suffix = "(备用)" if i == 3 else ""
            self.plc_variables[f"turntable_frequency_{i}"] = PLCVariable(
                name=f"转盘{i}运行频率{suffix}",
                address=f"DB100.DBD{(i-1)*4}",
                data_type="REAL",
                description=f"{i}号活性炭转盘运行频率",
                unit="Hz",
                min_value=0.0,
                max_value=50.0,
                default_value=25.0 if i < 3 else 0.0,
                read_only=False
            )
            self.plc_variables[f"turntable_running_{i}"] = PLCVariable(
                name=f"转盘{i}运行状态",
                address=f"DB100.DBX20.{i-1}",
                data_type="BOOL",
                description=f"{i}号转盘运行状态",
                read_only=True
            )
    ...
    def _add_monitoring_knowledge(self):
        """添加监测变量知识"""
        monitoring_vars = [
            ("inlet_toxicity", "进水毒性", "DB400.DBD0", "TU", 0.0, 100.0),
            ("outlet_toxicity", "出水毒性", "DB400.DBD4", "TU", 0.0, 100.0),
            ("inlet_flow", "进水流量", "DB400.DBD8", "m³/h", 0.0, 1000.0),
            ("inlet_ammonia", "进水氨氮", "DB400.DBD12", "mg/L", 0.0, 100.0),
            ("inlet_ph", "进水pH", "DB400.DBD16", "", 0.0, 14.0),
            ("water_temperature", "水温", "DB400.DBD20", "°C", 0.0, 50.0),
        ]
        
        for var_name, name, address, unit, min_val, max_val in monitoring_vars:
            self.plc_variables[var_name] = PLCVariable(
                name=name,
                address=address,
                data_type="REAL",
                description=name,
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "equipments": {
                k: {**v.__dict__} for k, v in self.equipments.items()
            },
